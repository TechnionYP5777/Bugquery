...
public class StackTraceRetrieverTest {
	...
	public void retrieverReturnsNullForIllegalParameters() {
	  String stackTrace = "Exception in thread \"main\" java.lang.NullPointerException\n" + 
		...
		assertNull(StackTraceRetriever.getMostRelevantStackTraces(null, new StackTrace(stackTrace), new JaccardSTDistancer(), 1));
		assertNull(StackTraceRetriever.getMostRelevantStackTraces(new ArrayList<Post>(), null, new JaccardSTDistancer(), 1));
		assertNull(StackTraceRetriever.getMostRelevantStackTraces(new ArrayList<Post>(), new StackTrace(stackTrace), null, 1));
		assertNull(StackTraceRetriever.getMostRelevantStackTraces(new ArrayList<Post>(), new StackTrace(stackTrace), new JaccardSTDistancer(), 0));
	}
